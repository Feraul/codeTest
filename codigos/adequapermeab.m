function[elem,kmap,normKmap,u,bedge,fonte,vel]=adequapermeab(benchmark, kmap,elem,bedge)
global centelem coord inedge normals
normKmap=0;
vel=0;
u=0;
fonte=0;
switch benchmark
    ...
    case 'herbin'
        ...
        for ianal = 1:size(bedge,1)
            ...
            %         cos(pi*overedgecoord(ianal,2))*sin(pi*overedgecoord(ianal,1)));
            %         0];
           IJ=coord(bedge(ianal,2),:)-coord(bedge(ianal,1),:);
            norma=norm(IJ);
            nij=R*IJ'/norma;
            
            auxpoint=(coord(bedge(ianal,2),:)+coord(bedge(ianal,1),:))*0.5;
            x=auxpoint(1,1);
            y=auxpoint(1,2);
        ...
        for ianal = 1:size(inedge,1)
             IJ=coord(inedge(ianal,2),:)-coord(inedge(ianal,1),:);
            norma=norm(IJ);
            nij=R*IJ'/norma;           
            auxpoint=(coord(inedge(ianal,2),:)+coord(inedge(ianal,1),:))*0.5;
            x=auxpoint(1,1);
            y=auxpoint(1,2);
    ...
    case 'heterogeneo'
        ...
        %                     u(ianal,1) = P1 + (centelem(ianal,1)*d*k2)/k1;
        %                     kmap(1,1:5)=[1 k1 0 0 k1];
        %                     elem(ianal,5)=1;
        %                 %Second material
        %                 elseif centelem(ianal,1) >= 1
        %                     u(ianal,1) = P2 - d*(L -centelem(ianal,1));
        %                     kmap(2,1:5)=[2 k2 0 0 k2];
        %                     elem(ianal,5)=2;
        %                 end  %End of IF
        %             end  %End of FOR
        %             fonte=0;
    ...
    case 'lepotier'
        ...
        for i=1:size(elem,1)
            ...
            %fonte(i,1)=-uu*((1+alfa)*pi^2*(x^2+y^2))-r*((1-3*alfa)*pi*x)-s*((1-3*alfa)*pi*y)-...
